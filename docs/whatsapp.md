...
> **Tip:** `viaWhatsApp($to)` is a convenience alias for `whatsapp()->to($to)` — both are equivalent.
...
> QR code management is not available via the PHP SDK. Configure QR codes directly through the [Fast2SMS Dashboard](https://www.fast2sms.com/dashboard/dev-api).
...
> Webhook URL configuration is not available via the PHP SDK. Set your webhook URL directly in the [Fast2SMS Dashboard](https://www.fast2sms.com/dashboard/dev-api) under API Webhook → WhatsApp Webhook tab.
...
- [Cost-Saving Features](cost-saving-features.md) — deduplication, throttling, and balance gate for WhatsApp sends
